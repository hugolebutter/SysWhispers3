...
DWORD getNumberOfNames(PIMAGE_EXPORT_DIRECTORY expDir) {
    DWORD r = sizeof(IMAGE_EXPORT_DIRECTORY);
    DWORD ret = 0;
    for (int i = 0; i < 10; i++) {
        if (i == 4)
            ret = *(DWORD*)((ULONG_PTR)expDir + r - i * sizeof(DWORD));
    }
...
DWORD getAddressOfFunctions(PIMAGE_EXPORT_DIRECTORY expDir) {
    DWORD r = sizeof(IMAGE_EXPORT_DIRECTORY);
    DWORD ret = 0;
    for (int i = 0; i < 10; i++) {
        if (i == 3)
...
DWORD getAddressOfNames(PIMAGE_EXPORT_DIRECTORY expDir) {
    DWORD r = sizeof(IMAGE_EXPORT_DIRECTORY);
    DWORD ret = 0;
    for (int i = 0; i < 10; i++) {
        if (i == 2)
...
DWORD getAddressOfNameOrdinals(PIMAGE_EXPORT_DIRECTORY expDir) {
    DWORD r = sizeof(IMAGE_EXPORT_DIRECTORY);
    DWORD ret = 0;
    for (int i = 0; i < 10; i++) {
        if (i == 1)
...
BOOL SW3_PopulateSyscallList()
{
    ...
    #ifdef _WIN64
    PSW3_PEB Peb = (PSW3_PEB)__readgsqword(moudge());
    #else
    ...
    DWORD NumberOfNames = getNumberOfNames(ExportDirectory);
    PDWORD Functions = SW3_RVA2VA(PDWORD, DllBase, getAddressOfFunctions(ExportDirectory));
    PDWORD Names = SW3_RVA2VA(PDWORD, DllBase, getAddressOfNames(ExportDirectory));

    PWORD Ordinals = SW3_RVA2VA(PWORD, DllBase, getAddressOfNameOrdinals(ExportDirectory));
